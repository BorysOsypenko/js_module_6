const keyTrainer = {
	...
	setCharCount: function() {
		return this.charCount = keyTrainer.checkPositiveInteger(prompt('Введите кол-во символов: '));
	},
	checkPositiveInteger: function(number) {
		while(true){
			if (parseInt(number) > 0 && Number.isInteger(parseInt(number))) {
				...
				return keyTrainer.checkPositiveInteger(prompt('Введите кол-во символов: '));
			}
		}
	},
	createTask: function() {
	...
	startTask: function() {
		...
		if (this.userErrors == null) {
			return console.log('Ого! Без ошибок!');
		} else {
			return console.log('Вы допустили ' + this.userErrors + ' ошибок. Старайтесь!')
		}
	}
};
